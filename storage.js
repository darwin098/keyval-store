...
module.exports.add = function add(data, key, expireAfterDays = 7) {
  const expireOn = getTimestampAfterNDays(expireAfterDays);
  if (!key) {
    return query(
      `INSERT INTO ${TABLE_NAME} (key, data, expire_on) VALUES($1, $2, $3) RETURNING key`,
      [nanoid(), JSON.stringify(data), expireOn]
    )
...
module.exports.get = function get(key, now = getTimestampAfterNDays(0)) {
  return query(
    `SELECT data FROM ${TABLE_NAME} where key = $1 and expire_on > $2;`,
    [key, now]
  ).then((result) => {
...
module.exports.deletePrev = function deletePrev() {
  return query(`DELETE FROM ${TABLE_NAME} where expire_on < $1;`, [
    getTimestampAfterNDays(7),
